...
func Run(dialAddr string) error {
	...
	err = myapi.RegisterCommonServiceHandler(context.Background(), gwmux, conn)
	if err != nil {
	...
	if err != nil {
		log.Fatalf("Failed to load a self-signed certtificate server.crt and server.key:", err)
	}
